...
*  uppercase and lowercase Latin letters. Now Petya wants to compare those two strings lexicographically.
* The letters' case does not matter, that is an uppercase letter is considered equivalent to the corresponding
...
* If the strings are equal, print "0". Note that the letters' case is not taken into consideration when the strings
* are compared.
...
public class PetyaAndStrings {
    public static void main(String[] args) {
        ...
        String str = sc.next();
        String str1 = sc.next();
        int comparison = compareString(str, str1);
        System.out.println(comparison);
    }

    private static int compareString(String str, String str1) {
        str = str.toLowerCase();
        str1 = str1.toLowerCase();
        for (int i = 0; i < str.length(); i++){
            if (str.charAt(i) < str1.charAt(i))
                return -1;
            else if (str.charAt(i) > str1.charAt(i))
                return 1;
        }
        return 0;
    }
