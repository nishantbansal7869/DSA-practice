...
*  At the i-th stop a i passengers exit the tram, while b i passengers enter it. The tram is empty before it
...
Then n lines follow, each contains two integers a i and b i (0 ≤ a i, b i ≤ 1000) — the number of passengers that
* exits the tram at the i-th stop, and the number of passengers that enter the tram at the i-th stop.
...
*  before it arrives at the stop. More formally, . This particularly means that a 1 = 0.
...
Since the number of passengers inside the tram never exceeds 6, a capacity of 6 is sufficient. Furthermore
* it is not possible for the tram to have a capacity less than 6. Hence, 6 is the correct answer.
...
public class Tram {
    public static void main(String[] args) {
        ...
        for(int i = 0; i < n; i++){
            ...
            int onbaarding = sc.nextInt();
            minimunseat = seat(exiting, onbaarding);
        }
        System.out.println(minimunseat);
    }
    static int currentPassenger = 0;
    static int totalSeat = 0;
    private static int seat(int exiting, int onbaarding) {
        currentPassenger = currentPassenger - exiting + onbaarding;
        totalSeat = Math.max(totalSeat, currentPassenger);
        return totalSeat;
    }
}
